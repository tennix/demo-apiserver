...
import (
	...
	"github.com/pingcap/apiserver/pkg/client/informers_generated/externalversions"
...
func (si *SharedInformers) startInformers(shutdown <-chan struct{}) {
	go si.Factory.Core().V1alpha1().TidbBackups().Informer().Run(shutdown)
	go si.Factory.Core().V1alpha1().TidbInstances().Informer().Run(shutdown)
}
